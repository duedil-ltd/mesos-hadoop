...
public class MesosSchedulerDriver implements SchedulerDriver, EventListener {
    ...
    public String getVersion(URI master) {
        if (master != null) {
            ...
            LOG.debug("Checking version at " + Utils.versionEndpoint(master));
            GenericUrl url = new GenericUrl(Utils.versionEndpoint(master));
            try {
...
//        }
    ...
    private void send(Call body) {
        ...
        GenericUrl url = new GenericUrl(Utils.schedulerEndpoint(this.masterUri));
